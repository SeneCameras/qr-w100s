...
class LKProcess(Process):
    def __init__(self, bufferqueue, velq):
        Process.__init__(self)
        self.bufferqueue = bufferqueue
        self.velocityqueue = velq
        self.running = True
        self.track_len = 10
        ...
        while self.running:
            try:
                ...
                if len(self.tracks) > 0:
                    ...
                    if len(deltay)>0:
                        #pass
                        self.velocityqueue.put((deltax, deltay, roll))
                        #print "avg:",(sum(deltax)/len(deltax),sum(deltay)/len(deltay)), "roll", (sum(roll)/len(roll))
            #            print deltax
            #           print deltay
